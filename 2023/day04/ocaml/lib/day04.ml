...
let parse_numbers (l : string) =
  ...
  |> filter (fun s -> String.length s > 0)
  |> map (int_of_string << String.trim)

let parse_line l =
  let split =
    nth (String.split_on_char ':' l) 1
    |> String.split_on_char '|' |> map String.trim
  in
...
let part1_lines lines =
  let rec aux lines acc =
    ...
    | l :: ls ->
        ...
        let l_points =
          filter (fun i -> contains i winners) owned |> length |> points
        in
        aux ls (acc + l_points)
  in
  aux lines 0
...
let common f input = String.trim input |> String.split_on_char '\n' |> f
